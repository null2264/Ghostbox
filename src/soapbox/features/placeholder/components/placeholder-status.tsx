...
const PlaceholderStatus: React.FC<IPlaceholderStatus> = ({ showPrivacyIndicator = false, variant }) => {
  const renderStatusInfo = () => showPrivacyIndicator ? (
    <StatusInfo visibility='placeholder' />
  ) : null;

  return (
    <div
      className={clsx({
        'status-placeholder bg-white dark:bg-primary-900 select-none': true,
