...
const MarkupDropdownMenu: React.FC<IMarkupDropdownMenu> = ({ style, items, placement, value, onClose, onChange }) => {
  ...
  return (
    <Motion defaultStyle={{ opacity: 0, scaleX: 0.85, scaleY: 0.75 }} style={{ opacity: spring(1, { damping: 35, stiffness: 400 }), scaleX: spring(1, { damping: 35, stiffness: 400 }), scaleY: spring(1, { damping: 35, stiffness: 400 }) }}>
      {({ opacity, scaleX, scaleY }) => (
        ...
        // react-overlays
        <div className={clsx('markup-dropdown__dropdown', placement)} style={{ ...style, opacity: opacity, transform: mounted ? `scale(${scaleX}, ${scaleY})` : undefined }} role='listbox' ref={node}>
          {items.map(item => (
            <div role='option' tabIndex={0} key={item.value} data-index={item.value} onKeyDown={handleKeyDown} onClick={handleClick} className={clsx('markup-dropdown__option', { active: item.value === value })} aria-selected={item.value === value} ref={item.value === value ? focusedItem : null}>
              <div className='markup-dropdown__option__icon'>
                <Icon src={item.icon} />
              </div>
...
interface IMarkupDropdown {
  composeId: string
}

const MarkupDropdown: React.FC<IMarkupDropdown> = ({
  composeId,
}) => {
  ...
  const value = compose.content_type;
  ...
  return (
    <div className={clsx('markup-dropdown', placement, { active: open })} onKeyDown={handleKeyDown} ref={node}>
      <div className={clsx('markup-dropdown__value', { active: valueOption && options.indexOf(valueOption) === 0 })}>
        <IconButton
          className={clsx({
